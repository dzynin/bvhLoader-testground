...
class BVHExporter {
  ...
  validateInputs(skeleton, clip) {
    ...
    //refactor
    const fliteredTracks = clip.tracks.filter(
      (track) => track instanceof VectorKeyframeTrack
    );

    const fliteredSkeletonBones = skeleton.bones.filter(
      (bone) => bone.name !== "ENDSITE"
    );

    // compare both hierarchy
    for (let i = 0; i < fliteredTracks.length; i++) {
      // I saw three.js's classes using let so It's should be fine.
      if (
        !fliteredTracks[i].name.match(/\[(.*?)\]/)[1] ===
        fliteredSkeletonBones[i].name
      )
        throw new Error(
          "Unmatch hierarchies of clip and skeleton, cannot process."
        );
    }
  }
  ...
  parseHierarchyBone(bone, skeleton, identLevel) {
    ...
    if (bone.name === skeleton.bones[0].name) {
      // checks if it's root
    ...
    if (bone.name === "ENDSITE") {
      ...
      this.writeLine(
        `${indent}  OFFSET ${bone.position?.x ?? 0} ${bone.position?.y ?? 0} ${
          bone.position?.z ?? 0
        }`
      );
      if (bone.children.length > 0) {
  ...
  parseMotionFrame(tracks, bones, frameIndex) {
    ...
    fliteredBones.forEach((bone, index) => {
      ...
      if (index === 0 && frameIndex === 0) {
        ...
        line += `0 0 0 `;// first rotation of root has to be 0
      } else {
        ...
        const multiplyResult = initFrameQ.multiply(motionFrameQ);
        // console.log("multiplyResult", multiplyResult);
        let eulerAngles = new Euler(0, 0, 0, "ZXY");
        eulerAngles = eulerAngles.setFromQuaternion(multiplyResult);
        console.log("eulerAngles", eulerAngles);
        line += `${eulerAngles.z} ${eulerAngles.x} ${eulerAngles.y} `;
      }
    });
